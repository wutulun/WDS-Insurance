...
def buy_auto():
    ...
    return render_template('buy_auto.html', title='Buy-auto', form=form)
...
def buy_home():
    ...
    if form.validate_on_submit():
        ...
        add = 0
        num_of_ho = len(form.houses)
        for house in form.houses:  
            if house.swimming_pool.data == 'N':
                swimming_pool = None
            else:
                swimming_pool = house.swimming_pool.data
            home = Home(purchase_date=house.purchase_date.data, home_type=house.home_type.data,
                        purchase_value=house.purchase_value.data.quantize(Decimal('.01'), rounding=ROUND_UP),
                        home_area=house.home_area.data.quantize(Decimal('.01'), rounding=ROUND_UP),     
                        fire=house.fire.data, security=house.security.data,
                        swimming_pool=swimming_pool, basement=house.basement.data)
            insurance.homes.append(home)      
            
            if home.purchase_value >= 10000:
                add += 20
            if home.home_area >= 10000:
                add += 30
            if home.fire == 0:
                add += 10
            if home.security == 0:
                add += 10
            if home.swimming_pool != None:
                add += 5
            if home.swimming_pool == 'M':
                add += 10
            if home.basement == 1:
                add += 20
        
        # Set premium       
        insurance.premium = (127.10 + num_of_ho*50 + add) * month
